...
class RCNClient:
    ...
    def download(self, filter_xml, start_index=0, count=1000, attributes=None):
        ...
        QgsMessageLog.logMessage(
            f"[WFS] download request: , URL: {url}", 
            "PobieranieEGIB", Qgis.Info
        )
        for attempt in range(max_retries):
            try:
...
<wfs:GetFeature service="WFS" version="2.0.0" 
    ...
    <wfs:Query typeNames="ms:{self.obj_layer}" srsName="EPSG:2180">
    ...
    def build_id_filter(self, ids):
        ...
        for identifier in ids:
            ...
            if '*' in identifier or '?' in identifier:
                 conditions.append(f'<fes:PropertyIsLike wildCard="*" singleChar="?" escape="\\"><fes:ValueReference>dzi_id_dzialki</fes:ValueReference><fes:Literal>{identifier}</fes:Literal></fes:PropertyIsLike>')
            else:
                conditions.append(f'<fes:PropertyIsEqualTo><fes:ValueReference>dzi_id_dzialki</fes:ValueReference><fes:Literal>{identifier}</fes:Literal></fes:PropertyIsEqualTo>')
